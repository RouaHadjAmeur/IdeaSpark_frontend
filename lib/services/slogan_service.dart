...
class SloganService {
  ...
  /// Génère des slogans à partir d'un prompt brut (Agent Full Access)
  static Future<List<SloganModel>> generateSlogansFromPrompt({
    required String prompt,
  }) async {
    try {
      final authService = AuthService();
      ...
      final Map<String, dynamic> requestBody = {
        'brandName': 'Agent AI',
        'language': 'fr',
        'copywritingPrompt': prompt,
      };

      print('🚀 Slogans via Prompt URL: $url');
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        try {
          final match = RegExp(r"models/([\w-]+)").firstMatch(body);
          if (match != null && body.contains('not found')) {
            print('⚠️ Backend attempted to use unsupported model. Falling back to mock slogans.');
            return MockSloganData.generateMockSlogans();
          }
        } catch (_) {}
        throw Exception('Failed to generate slogans: ${response.statusCode} - $body');
      ...
      print('⚠️ Erreur Slogans via Prompt: $e');
      final msg = e.toString();
